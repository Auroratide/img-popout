...
export default class ImgPopoutElement extends HTMLElement {
    ...
    hideCover() {
        ...
        if (this.transition.out)
            this.runTransition(this.transition.out(this.transition.in(this.elements.cover, this.poppedOutImage(), this.mainImage())))
        else
    ...
    poppedOutImage() {
        const elems = this.elements.poppedOutSlot.assignedElements()
        return elems.length > 0 ? elems[0] : null
    }
}
